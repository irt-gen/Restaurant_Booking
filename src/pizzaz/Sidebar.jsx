...
import { Filter, Settings2, Star, Clock } from "lucide-react"; // Added Clock
import { AnimatePresence, motion } from "framer-motion";

function PlaceListItem({ place, isSelected, onClick }) {
  // Get today's hours
  const today = new Date().toLocaleDateString("en-US", { weekday: "short" });
  const todayHours = place.hours ? place.hours[today] : null;
        ...
        >
          ...
          <div className="min-w-0 flex flex-col gap-0.5">
            ...
            <div className="text-xs text-black/50 flex items-center gap-1">
              <Star className="h-3 w-3 fill-black/50 text-transparent" aria-hidden="true" />
              <span className="text-black font-medium">{place.rating.toFixed(1)}</span>
              {place.price ? <span>· {place.price}</span> : null}
            </div>

            {/* Added Time Display */}
            {todayHours && (
               <div className="text-xs text-green-700 flex items-center gap-1 font-medium mt-1">
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div className="flex justify-between flex-row items-center px-3 sticky bg-white top-0 py-4 text-md font-medium z-10">
